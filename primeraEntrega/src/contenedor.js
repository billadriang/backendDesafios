...
class Contenedor {
  ...
  async _readFileOrCreateNewOne() {
    try {
      ...
      error.code === "ENOENT"
        ...
        : console.log(
            `Codigo de error: ${error.code} | hubo un error inesperado al leer ${this._filename}`
          );
    }
  }
  ...
  async _createEmptyFile() {
    fs.writeFile(this._filename, "[]", (error) => {
      error
        ? console.log(error)
        : console.log(` ${this._filename} fue creado`);
    });
  }
  ...
  async getById(id) {
    ...
    try {
      ...
      console.log(
        `Codigo de error ${error.code} | error al intentar obtener con este id (${id})`
      );
    }
  }
  ...
  async addToArrayById(id, objectToAdd) {
    ...
    try {
      ...
      if (objectIdToBeUpdated) {      
        ...
        console.log(`ID ${id} No eiste en el archivo`);
        return false;
      }

    } catch (error) {
      `Codigo de error ${error.code} | error al intentar actualizar con este id (${id})`
    }
    } else {
      return false;
    }
  ...
  async save(object) {    
    if(this._validateKeysExist(object)) {
      try {
        ...
        console.log(
          `Codigo de error ${error.code} | error al intentar guardar un elemento `
        );
      }
    } else {
      return false;
  ...
  async deleteAll() {
    try {
      ...
      console.log(
        `hubo un error (${error.code}) al tratar de eliminar todos los objetos`
      );
    }
  }
